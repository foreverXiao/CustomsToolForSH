...
{
    public partial class GetBeiAnDanDataForm_RM_D : Form
    {
        ...
        private static GetBeiAnDanDataForm_RM_D BADDataFrm_RMD;
        public GetBeiAnDanDataForm_RM_D()
        {
            InitializeComponent();
        }

        public static GetBeiAnDanDataForm_RM_D CreateInstance()
        {
            if (BADDataFrm_RMD == null || BADDataFrm_RMD.IsDisposed)
            {
                BADDataFrm_RMD = new GetBeiAnDanDataForm_RM_D();
            }
            return BADDataFrm_RMD;
        }

        private void GetBeiAnDanDataForm_RM_D_Load(object sender, EventArgs e)
        {
            ...
            dtRatio = sqlLib.GetDataTable(@"SELECT [Rate] FROM B_ExchangeRate WHERE [Object] = 'RMB:USD'", "B_ExchangeRate").Copy();
            dtHsCode = sqlLib.GetDataTable(@"SELECT DISTINCT [CHN Name], [Duty Rate] FROM B_HS", "B_HS").Copy();
            sqlLib.Dispose(0);
        }

        private void GetBeiAnDanDataForm_RM_D_FormClosing(object sender, FormClosingEventArgs e)
        {
            this.dtGatherData.Dispose();
            this.dtExtractDoc.Dispose();
            this.dtRatio.Dispose();
        ...
        {
            this.btnDL.Visible = false;
            string strSQL = @"SELECT P.[Group ID], P.[GongDan No], P.[PingDan Qty], P.[FG EHB], P.[FG CHN Name] AS [CHN Name], P.[Order No], P.[Order Category], 
                              P.[ESS/LINE], CAST(P.[PingDan Amount] / P.[PingDan Qty] AS decimal(18, 4)) AS [Selling Price], P.[PingDan Amount] AS [Selling Amount], 
                              G.[Created Date] AS [Pass to IE Date], G.[Approved Date] AS [GongDan Approved Date] FROM C_PingDan AS P INNER JOIN (SELECT [GongDan No], 
                              [Created Date], [Approved Date] FROM C_GongDan WHERE [IE Type] = 'RM-D') AS G ON P.[GongDan No] = G.[GongDan No] WHERE P.[IE Type] = 'RM-D'  
                              AND [PingDan ID] IS NOT NULL AND [PingDan No] IS NOT NULL AND [Pass Gate Time] IS NOT NULL AND P.[GongDan No] NOT IN (SELECT [GongDan No] 
                              FROM C_BeiAnDan WHERE [IE Type] = 'RM-D') ORDER BY P.[Group ID], P.[GongDan No]";
            string strDoc = @"SELECT P.[Group ID], '' AS [备案单号], N'成品内销（料件）' AS [单证类型], 0.0 AS [总毛重], N'保区进料料件' AS [贸易方式], '' AS [结转企业代码], 
                              0 AS [项号], P.[FG EHB] AS [备件号], P.[GongDan Qty] AS [数量], P.[GongDan Qty] AS [净重], 0.0 AS [毛重], P.[PingDan Amount] AS [金额], 
                              ...
                              [PingDan ID] IS NOT NULL AND [PingDan No] IS NOT NULL AND [Pass Gate Time] IS NOT NULL AND P.[GongDan No] NOT IN (SELECT [GongDan No] FROM 
            ...
            sqlComm.CommandText = @"usp_GatherBeiAnDanforRM-D";
            ...
            sqlComm.Parameters.Clear();
            sqlComm.CommandText = @"usp_InsertBeiAnDanforRM-D";
            ...
            this.dgvBeiAnDan.Columns["CHN Name"].Frozen = true;
        ...
        private void btnExtractDoc_Click(object sender, EventArgs e)
        {
            ...
            {
                sqlComm.CommandText = @"usp_InsertBeiAnDanDocforRM-D";
                ...
                sqlComm.Parameters.AddWithValue("@TVP_BAD_DOC", dtExcDoc);
        ...
        {
            if (this.dtExtractDoc.Rows.Count == 0) { MessageBox.Show("Please click [Extract Doc.] button first.", "Prompt", MessageBoxButtons.OK, MessageBoxIcon.Stop); return; }
            ...
            DataTable dtBAD_DOC = lib.SelectDistinct(dtExtractDoc, strFields);
            lib.Dispose(0);
            dtBAD_DOC.Columns.Add("数量", typeof(string));
            dtBAD_DOC.Columns.Add("毛重", typeof(string));
            dtBAD_DOC.Columns.Add("金额", typeof(string));
            foreach (DataRow dr in dtBAD_DOC.Rows)
            {
            ...
            worksheet_Doc.get_Range(worksheet_Doc.Cells[1, 1], worksheet_Doc.Cells[dtBAD_DOC.Rows.Count + 1, 14]).NumberFormatLocal = "@";
            string strRQ = System.DateTime.Now.ToString("yyyyMMdd");

            for (int a = 0; a < dtBAD_DOC.Rows.Count; a++)
            {
                worksheet_Doc.Cells[a + 2, 1] = dtBAD_DOC.Rows[a]["Group ID"].ToString().Trim();
                ...
                worksheet_Doc.Cells[a + 2, 8] = dtBAD_DOC.Rows[a]["备件号"].ToString().Trim();
                worksheet_Doc.Cells[a + 2, 9] = dtBAD_DOC.Rows[a]["数量"].ToString().Trim();
                worksheet_Doc.Cells[a + 2, 10] = dtBAD_DOC.Rows[a]["数量"].ToString().Trim();
                worksheet_Doc.Cells[a + 2, 11] = dtBAD_DOC.Rows[a]["毛重"].ToString().Trim();
                worksheet_Doc.Cells[a + 2, 12] = dtBAD_DOC.Rows[a]["金额"].ToString().Trim();
            ...
            worksheet_Doc.get_Range(worksheet_Doc.Cells[1, 1], worksheet_Doc.Cells[dtBAD_DOC.Rows.Count + 1, 14]).Font.Name = "Verdana";
            worksheet_Doc.get_Range(worksheet_Doc.Cells[1, 1], worksheet_Doc.Cells[dtBAD_DOC.Rows.Count + 1, 14]).Font.Size = 8;
            worksheet_Doc.Cells.EntireColumn.AutoFit();
        ...
        private void btnPreview_Click(object sender, EventArgs e)
        {
            ...
            if (!String.IsNullOrEmpty(this.dtpTo.Text.Trim()))
            {
                if (!String.IsNullOrEmpty(this.dtpFrom.Text.Trim()))
                {
                    ...
                    { strBrowse += " AND B.[BeiAnDan Date] >= '" + Convert.ToDateTime(this.dtpFrom.Value.ToString("M/d/yyyy")) + "' AND B.[BeiAnDan Date] < '" + Convert.ToDateTime(this.dtpTo.Value.AddDays(1.0).ToString("M/d/yyyy")) + "'"; }
                }
                else
                { strBrowse += " AND B.[BeiAnDan Date] < '" + Convert.ToDateTime(this.dtpTo.Value.AddDays(1.0).ToString("M/d/yyyy")) + "'"; }
            }
            else
            {
                if (!String.IsNullOrEmpty(this.dtpFrom.Text.Trim()))
            ...
            string strSQL = @"SELECT B.[Group ID], B.[GongDan No], B.[GongDan Qty], B.[FG EHB], B.[CHN Name], B.[Order No], B.[Order Category], B.[ESS/LINE], 
                              B.[Selling Price], B.[Selling Amount], B.[Pass to IE Date], B.[GongDan Approved Date], B.[BeiAnDan ID], B.[BeiAnDan No], B.[BeiAnDan Date], 
                              B.[Creater], B.[Tax & Duty Paid Date], B.[Customs Release Date], P.[PingDan ID] FROM C_BeiAnDan AS B INNER JOIN C_PingDan AS P ON 
                              B.[GongDan No] = P.[GongDan No] WHERE" + strBrowse + " ORDER BY B.[Group ID], B.[GongDan No]";
            SqlConnection sqlConn = new SqlConnection(SqlLib.StrSqlConnection);
            if (sqlConn.State == ConnectionState.Closed) { sqlConn.Open(); }
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (bExist == false)
            {
                ...
                {
                    decimal dRate = Convert.ToDecimal(dtRatio.Rows[0][0].ToString().Trim());
            ...
            Comm.CommandText = @"usp_UpdateBeiAnDanforRM-D";
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Already 2nd release, reject to delete the data.", "Prompt", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                return;
            }
            ...
            sqlComm.Connection = sqlConn;
            sqlComm.CommandType = CommandType.StoredProcedure;
            sqlComm.CommandText = @"usp_DeleteDataForHistoricalBeiAnDan";
        ...
        private void btnReport_Click(object sender, EventArgs e)
        {
            ...
            sqlComm.CommandType = CommandType.StoredProcedure;
            sqlComm.CommandText = @"usp_BeiAnDan_BOMReport_RMD";
            sqlComm.Parameters.AddWithValue("@GroupID", strGroupID);
            ...
            sqlComm.CommandType = CommandType.StoredProcedure;
            sqlComm.CommandText = @"usp_BeiAnDan_BGDReport_RMD";
            sqlAdapter.SelectCommand = sqlComm;
            ...
            sqlComm.CommandText = @"usp_BeiAnDan_InvoiceUSDReport_Mid_RMD";
            sqlAdapter.SelectCommand = sqlComm;
            ...
            sqlComm.CommandText = @"usp_BeiAnDan_InvoiceUSDReport_RMD";
            sqlAdapter.SelectCommand = sqlComm;
            ...
            sqlComm.CommandText = @"usp_BeiAnDan_InvoiceRMBReport_RMD";
            sqlAdapter.SelectCommand = sqlComm;
            ...
            sqlComm.CommandText = @"usp_BeiAnDan_GongDanInfoReport_RMD";
            sqlAdapter.SelectCommand = sqlComm;
